...
def test_procedure_optimization_query(storage_socket):
    ...
    meta, procs = storage_socket.procedure.optimization.query(created_before=datetime.utcnow(), status=["INCOMPLETE"])
    ...
    meta, procs = storage_socket.procedure.optimization.query(status=["COMPLETE"], include=["*", "stdout_obj"])
    assert meta.n_returned == 1
    assert len(procs[0]["stdout_obj"]) > 1

    meta, procs = storage_socket.procedure.optimization.query(status=["COMPLETE"], include=["*", "trajectory_obj"])
    assert meta.n_returned == 1
