...
def test_queue_error(fractal_test_server):
    ...
    queue_ret = storage_socket.tasks.query_tasks(status=[RecordStatusEnum.error])[1]
    result = storage_socket.records.query(id=compute_ret.ids)[1][0]
...
def test_queue_query_manager(fractal_test_server):
    ...
    storage_socket.tasks.claim_tasks(manager_name, {"rdkit": None}, limit=1)[0]
