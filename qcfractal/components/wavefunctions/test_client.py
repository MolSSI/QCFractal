...
def test_wavefunction_client_basic(storage_socket: SQLAlchemySocket, snowflake_client: PortalClient):
    ...
    r = snowflake_client.get_wavefunctions([added_ids[1], added_ids[1], added_ids[0]], missing_ok=False)
    assert len(r) == 3
    ...
    r = snowflake_client.get_wavefunctions(added_ids[1])
    assert isinstance(r, WavefunctionProperties)
    assert_wfn_equal(r, wfn2)


def test_wavefunction_client_get_nonexist(storage_socket: SQLAlchemySocket, snowflake_client: PortalClient):
    # Add via the storage socket - the client doesn't allow for adding wavefunctions
    wfn1 = load_wavefunction_data("psi4_peroxide")
    wfn2 = load_wavefunction_data("psi4_fluoroethane")
    ...
    # Now get via the client
    r = snowflake_client.get_wavefunctions([added_ids[1], 999, added_ids[0]], missing_ok=True)
    assert len(r) == 3
    assert_wfn_equal(wfn2, r[0])
    assert_wfn_equal(wfn1, r[2])
    assert r[1] is None
...
def test_outputs_client_get_empty(storage_socket: SQLAlchemySocket, snowflake_client: PortalClient):
    ...
    out = snowflake_client.get_wavefunctions([])
    assert out == []
