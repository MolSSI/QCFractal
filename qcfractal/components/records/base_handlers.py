...
if TYPE_CHECKING:
    ...
    from qcfractal.components.services.db_models import ServiceDependenciesORM, ServiceQueueORM
...
class BaseServiceHandler(abc.ABC):
    ...
    def submit_subtasks(
        ...
        for key, molecule, spec in task_inputs:
            meta, added_ids = self.root_socket.tasks.create([molecule], spec, session=session)

            if not meta.success:
                raise RuntimeError("Problem submitting task: {}.".format(meta.error_string))

            service_task = ServiceDependenciesORM(procedure_id=added_ids[0], service_id=service_orm.id, extras=key)  # type: ignore
