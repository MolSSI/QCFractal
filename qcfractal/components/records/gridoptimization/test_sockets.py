...
def test_gridoptimization_socket_query(storage_socket: SQLAlchemySocket):
    ...
    # query for basis
    meta, td = storage_socket.records.gridoptimization.query(GridoptimizationQueryBody(singlepoint_basis=["sTO-3g"]))
    assert meta.n_found == 3

    meta, td = storage_socket.records.gridoptimization.query(GridoptimizationQueryBody(singlepoint_basis=[None]))
    assert meta.n_found == 0

    meta, td = storage_socket.records.gridoptimization.query(GridoptimizationQueryBody(singlepoint_basis=[""]))
    assert meta.n_found == 0

    # query for method
    meta, td = storage_socket.records.gridoptimization.query(GridoptimizationQueryBody(singlepoint_method=["b3lyP"]))
    assert meta.n_found == 1

    kw_id = td[0]["specification"]["optimization_specification"]["singlepoint_specification"]["keywords_id"]
    meta, td = storage_socket.records.gridoptimization.query(GridoptimizationQueryBody(singlepoint_keywords_id=[kw_id]))
    assert meta.n_found == 3
