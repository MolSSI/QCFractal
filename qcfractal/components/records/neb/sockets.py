...
class NEBRecordSocket(BaseRecordSocket):
    ...
    def add_specification(
        self, td_spec: NEBSpecification, *, session: Optional[Session] = None
        ...
        with self.root_socket.optional_session(session, False) as session:
            ...
            meta, sp_spec_id = self.root_socket.records.singlepoint.add_specification(
                td_spec.qc_specification, session=session
            )
            if not meta.success:
                return (
                    InsertMetadata(
    ...
    def query(
        ...
        *,
        ...
        if query_data.initial_chain_id is not None:
            and_query.append(NEBChainsORM.neb_id.in_(query_data.initial_chain_id))
      ...
      #      stmt = stmt.join(
      #          NEBInitialMoleculeORM,
      #          NEBInitialMoleculeORM.neb_id == NEBRecordORM.id,
      #      )
        ...
        with self.root_socket.optional_session(session, False) as session:
            ...
            init_mol_cte = (
                select(
                    ...
                    array_agg(
                        aggregate_order_by(
                            NEBInitialchainORM.molecule_id, NEBInitialchainORM.molecule_id.asc()
                        )
                    ).label("molecule_ids"),
                )
                .join(
...
#                td_orm.optimizations.appen`d(opt_history)
