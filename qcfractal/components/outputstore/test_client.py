...
def test_outputs_client_basic_str(
    storage_socket: SQLAlchemySocket, fractal_test_client: PortalClient, compression, compression_level
):
    ...
    r = fractal_test_client._get_outputs(added_ids, missing_ok=False)
    assert len(r) == 2
    ...
    # get a single id
    r = fractal_test_client._get_outputs(added_ids[0])
    assert isinstance(r, OutputStore)


def test_outputs_client_get_nonexist(storage_socket: SQLAlchemySocket, fractal_test_client: PortalClient):
    input_dict = {str(k): "This is some input " * k for k in range(5)}
    added_ids = storage_socket.outputstore.add([input_dict, input_dict])

    out = fractal_test_client._get_outputs([added_ids[0], 999, added_ids[1]], missing_ok=True)
...
def test_outputs_client_get_empty(storage_socket: SQLAlchemySocket, fractal_test_client: PortalClient):
    ...
    out = fractal_test_client._get_outputs([])
    assert out == []
