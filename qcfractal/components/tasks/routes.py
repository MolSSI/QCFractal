...
def query_task_v1():
    ...
    meta, tasks = storage_socket.tasks.query_tasks(**{**data, **body.meta.dict()})
...
def add_task_v1():
    ...
    meta, ids = storage_socket.tasks.create(body.data, body.meta)
...
def modify_task_v1():
    ...
    if body.meta.operation == "restart":
        ...
        tasks_updated = storage_socket.tasks.reset_tasks(**d, reset_error=True)
        ...
        task_ids = storage_socket.tasks.regenerate_tasks(body.data.base_result, new_tag, new_priority)
        data = {"n_updated": len(task_ids) - task_ids.count(None)}

    elif body.meta.operation == "modify":
        tasks_updated = storage_socket.tasks.modify_tasks(
            id=body.data.id,
