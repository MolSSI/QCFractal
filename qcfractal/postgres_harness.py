...
class PsqlHarness:
    ...
    def initialize(config):
        ...
        init_code, init_stdout = _run([shutil.which("initdb"), "-D", config.database_path], logger=self.logger, quiet=self.quiet)
        ...
        start_code, start_stdout = _run([
            ...
            "start"],
            logger=self.logger, quiet=quiet) # yapf: disable
        ...
        def run_psql(cmd):
            ...
            return _run(psql_cmd + [cmd], logger=self.logger, quiet=quiet)

        success = self.create_database(config.database.default_database)
