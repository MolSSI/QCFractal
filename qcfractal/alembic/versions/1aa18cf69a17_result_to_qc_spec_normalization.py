...
def upgrade():
    ...
    unique_specs = session.query(result_table.c.program, result_table.c.driver,
                                 result_table.c.basis, result_table.c.method, result_table.c.keywords).distinct().all()
    # from sqlalchemy.dialects import postgresql
    # query = session.query(KeywordsORM.id).filter(KeywordsORM.values.cast(JSONB) == cast('{"args": "unknown"}',JSONB))
    # print (query.statement.compile(dialect=postgresql.dialect()))
    kw_dummy = session.query(KeywordsORM.id).filter(KeywordsORM.values.cast(JSONB) == cast({"args": "unknown"},JSONB)).first()
    kw_dummy = kw_dummy[0]
    ...
    for spec in unique_specs:
        ...
        query = session.query(QCSpecORM.id).filter(sa.and_
...
def downgrade():
    ...
    for spec_id in unique_spec_ids:
        spec = session.query(QCSpecORM.program, QCSpecORM.driver,
                      QCSpecORM.basis, QCSpecORM.method, QCSpecORM.keywords).filter(QCSpecORM.id == spec_id).first()
        update_cnt = session.query(result_table).filter(result_table.c.qc_spec == spec_id).update(
            {
            result_table.c.program : spec.program,
            result_table.c.driver : spec.driver,
            result_table.c.basis : spec.basis,
