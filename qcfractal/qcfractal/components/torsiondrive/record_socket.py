...
from qcportal.utils import hash_dict, is_included, reshape_molecule
...
class TorsiondriveRecordSocket(BaseRecordSocket):
    ...
    def initialize_service(
        ...
        initial_molecules: List[Molecule] = [x.molecule.to_model(Molecule) for x in td_orm.initial_molecules]
        ...
        molecule_template = initial_molecules[0].model_dump(exclude={"id", "identifiers"}, mode="json")
        ...
        with contextlib.redirect_stdout(td_stdout):
            td_state = td_api.create_initial_state(
                ...
                init_coords=[x.geometry.tolist() for x in initial_molecules],  # nested lists, NOT flattened
    ...
    def iterate_service(
        ...
        for task in complete_tasks:
            ...
            initial_mol_geom = reshape_molecule(mol_data[0]["geometry"])
            final_mol_geom = reshape_molecule(mol_data[1]["geometry"])
    ...
    def add_specifications(
        ...
        for td_spec in td_specs:
            td_kw_dict = td_spec.keywords.model_dump()
