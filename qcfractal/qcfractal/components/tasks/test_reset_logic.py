...
def test_reset_logic_dict_comprehension_bug(postgres_server, client_encoding):
    ...
    extra_config = {"auto_reset": {"enabled": True, "unknown_error": 2}}
    ...
    with QCATestingSnowflake(pg_harness, encoding=client_encoding, extra_config=extra_config) as snowflake:
        ...
        fop_bad_state = FailedOperation(
            error=ComputeError(error_type="BadStateException", error_message="QOSMaxSubmitJobPerUserLimit reached")
        )
        fop_too_many = FailedOperation(
            error=ComputeError(error_type="TooManyJobFailuresError", error_message="Wrapped Parsl exception")
        )
        ...
        with storage_socket.session_scope() as session:
            ...
            storage_socket.tasks.update_finished(
                activated_manager_name.fullname, {tasks[0]["id"]: compress_result(fop_bad_state.model_dump())}
            )
            session.expire(rec)
            ...
            for i in range(2):
                tasks = storage_socket.tasks.claim_tasks(
                    activated_manager_name.fullname, activated_manager_programs, ["*"]
                )
                print(f"Iteration {i}: rec.status = {rec.status}, history length = {len(rec.compute_history)}")
                assert len(tasks) == 1
                storage_socket.tasks.update_finished(
                    activated_manager_name.fullname, {tasks[0]["id"]: compress_result(fop_too_many.model_dump())}
                )
                session.expire(rec)
