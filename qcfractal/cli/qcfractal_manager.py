...
class CommonManagerSettings(AutodocBaseSettings):
    ...
    """
    adapter: AdapterEnum = Schema(AdapterEnum.pool,
    ...
    retries: int = Schema(
        ...
        ge=0)
    scratch_directory: Optional[str] = Schema(None, description="Scratch directory for Engine execution jobs.")
    verbose: bool = Schema(
        False,
...
class FractalServerSettings(AutodocBaseSettings):
    ...
    """
    fractal_uri: str = Schema("localhost:7777", description="Full URI to the Fractal Server you want to connect to")
    ...
    verify: Optional[bool] = Schema(None, description="Use Server-side generated SSL certification or not.")
...
def parse_args():
    ...
    common.add_argument("--adapter",
                        type=str,
                        help="The backend adapter to use, currently only {'dask', 'parsl', 'pool'} are valid.")
    common.add_argument(
        "--tasks-per-worker",
        type=int,
    ...
    manager.add_argument("--max-queued-tasks",
                         type=int,
                         help="Maximum number of tasks to hold at any given time. "
    ...
    optional.add_argument("--ntests",
                          type=int,
                          help="How many tests per found program to run, does nothing without --test set")
    optional.add_argument("--schema",
                          action="store_true",
                          ...
                          "JSON-like format.")
    optional.add_argument("--skeleton",
                          "--skel",
                          type=str,
...
def main(args=None):
    ...
    if settings.manager.test:
        ...
        client = qcfractal.interface.FractalClient(address=settings.server.fractal_uri,
                                                   **settings.server.dict(skip_defaults=True, exclude={"fractal_uri"}))
    ...
    manager = qcfractal.queue.QueueManager(client,
                                           queue_client,
