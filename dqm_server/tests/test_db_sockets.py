...
def test_return_value(mongo_socket):
    wpbe_val = mongo_socket.get_value(
        "return_value",
        "HBC6",
        "FaOOFaOO_0.9444444444444444",
        "cp",
        "B3LYP/aug-cc-pVDZ",
        do_stoich=True,
        debug_level=1)
...
def test_get_series_variable(mongo_socket):
    wpbe_val = mongo_socket.get_series(
        "variables.SCF N ITERS", "HBC6", "cp", "B3LYP/aug-cc-pVDZ", do_stoich=False, debug_level=1)
...
def test_get_series_return_value(mongo_socket):
    wpbe_val = mongo_socket.get_series(
        "return_value", "HBC6", "cp", "B3LYP/aug-cc-pVDZ", do_stoich=True, debug_level=1)
    assert (np.isclose(
        wpbe_val["B3LYP/aug-cc-pVDZ"]["FaOOFaOO_1.777777777777778"],
        -0.001889,
        rtol=1.e-6,
        atol=1.e-6) and np.isclose(
            wpbe_val["B3LYP/aug-cc-pVDZ"]["FaOOFaNN_1.0555555555555556"],
            -0.033706,
            rtol=1.e-6,
            atol=1.e-6))
...
def test_get_dataframe_return_value(mongo_socket):
    wpbe_val = mongo_socket.get_dataframe(
        "return_value",
        "HBC6",
        "cp", ["WPBE/qzvp", "B3LYP/aug-cc-pVDZ"],
        do_stoich=True,
        debug_level=1)
    assert np.isclose(wpbe_val["WPBE/qzvp"]["FaONFaON_1.075"], -13.0246, rtol=1.e-6, atol=1.e-6)
    assert np.isclose(
        wpbe_val["B3LYP/aug-cc-pVDZ"]["FaONFaON_1.075"], -0.020754, rtol=1.e-6, atol=1.e-6)


def test_get_dataframe_variable(mongo_socket):
    wpbe_val = mongo_socket.get_dataframe(
        "variables.CURRENT DIPOLE Y",
        "HBC6",
        "cp", ["WPBE/qzvp", "B3LYP/aug-cc-pVDZ"],
        do_stoich=True,
        debug_level=1)
    ...
    assert np.isclose(
        wpbe_val["B3LYP/aug-cc-pVDZ"]["FaONFaON_1.05"], 7.158318e-11, rtol=1.e-6, atol=1.e-6)
...
def test_add_remove_molecule(mongo_socket):
    ...
    del_hash = dqm.Molecule(data, dtype="json").get_hash()
    data["_id"] = del_hash

    result = mongo_socket.add_molecule(data)
    assert result
    result = mongo_socket.del_molecule_by_hash(del_hash)
    assert result
    result = mongo_socket.add_molecule(data)
    assert result
    ...
    assert result
    result = mongo_socket.del_molecule_by_hash(del_hash, "some_project")
    assert result
    result = mongo_socket.add_molecule(data, "some_project")
    assert result
...
def test_batch_remove(mongo_socket):
    ...
    result = mongo_socket.del_database_by_hash(
        ["7b3ce68b6c2f7d67dae4210eeb83be69f978e2a8", "205c97d9248d2cd12db1c55ba421eb8df84b22a7"])
    assert result == 2
    for item in batch:
        assert mongo_socket.add_database(item)
...
def test_list_methods(mongo_socket):
    ...
    mongo_socket.add_page(data)
    result = mongo_socket.list_methods([
        "92e8b7bebf5382d5056754e62e17993ef2b1b379", "8665e03700fd99259aa21c86fa2d2df11cb0dbef",
        "f9dec31ed1e51fbb850c6d113e3b4081eebf3792", "b"
    ])
...
def test_search_qc_return_value(mongo_socket):
    result = mongo_socket.search_qc_variable(
        ["af4c153199d3386e8bb1ff4780df955d2adeee80",
         "d560eb7fe48db28eb57902e4469f2a9b8af45c71"], "return_value")
...
def test_search_qc_variable(mongo_socket):
    result = mongo_socket.search_qc_variable(
        ["af4c153199d3386e8bb1ff4780df955d2adeee80",
         "d560eb7fe48db28eb57902e4469f2a9b8af45c71"], "variables.NUCLEAR REPULSION ENERGY")
...
def test_evaluate_return_value(mongo_socket):
    result = mongo_socket.evaluate(["efad0bae4a0bdf4aeea66b3c29ec505bdd61b2a1"],
                                   ["B3LYP/aug-cc-pVDZ", "Test"])
...
def test_evaluate_exhaustive_return_value(mongo_socket):
    ...
    mongo_socket.add_page(data)
    result = mongo_socket.evaluate(["efad0bae4a0bdf4aeea66b3c29ec505bdd61b2a1", "b", "und"],
                                   ["B3LYP/aug-cc-pVDZ", "a", "not available"])
    mongo_socket.del_page_by_hash("b8106d3072fd101cf33f937b0db5b73e670c1dd9")
...
def test_evaluate_exhaustive_variables(mongo_socket):
    data = {"_id": "NewPage", "modelchem": "a", "molecule_hash": "b", "return_value": "accessed"}
    mongo_socket.add_page(data)
    result = mongo_socket.evaluate(
        ["efad0bae4a0bdf4aeea66b3c29ec505bdd61b2a1", "b", "und"],
        ["B3LYP/aug-cc-pVDZ", "a", "not available"],
        field="variables.NUCLEAR REPULSION ENERGY")
    mongo_socket.del_page_by_hash("b8106d3072fd101cf33f937b0db5b73e670c1dd9")
    assert math.isnan(result.as_matrix()[0][0])
    assert math.isnan(result.as_matrix()[0][1])
    assert math.isnan(result.as_matrix()[0][2])
...
def test_evaluate_2_exhaustive(mongo_socket):
    data = {
        "_id": "NewPage",
        "modelchem": "B3LYP/aug-cc-pVDZ",
        "molecule_hash": "b",
        "return_value": "accessed"
    }
    mongo_socket.add_page(data)
    result = mongo_socket.evaluate_2([
        "efad0bae4a0bdf4aeea66b3c29ec505bdd61b2a1", "6f34560054454808dbd49c407de31f08b58dcbbe", "b",
        "und"
    ], ["return_value", "variables.NUCLEAR REPULSION ENERGY", "invalid"], "B3LYP/aug-cc-pVDZ")
    mongo_socket.del_page_by_hash("b8106d3072fd101cf33f937b0db5b73e670c1dd9")
