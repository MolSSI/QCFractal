...
def build_unique_mol(num=1):
    mols = [
        qcel.models.Molecule(symbols=["He", "He"], geometry=np.random.rand(2, 3) + i, validated=True)
        for i in range(num)
    ]
...
def create_unique_result(mols):
    results = [
        ResultRecord(version="1", driver="energy", program="games", molecule=mol_ret, method="test", basis="6-31g")
        for mol_ret in mols
    ]
