...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Copy a sample of a production QCArchive database into a staging or development database"
    )
    ...
    production_storage = storage_socket_factory(
        args.src, db_type="sqlalchemy", max_limit=args.maxlimit, skip_version_check=False
    )
    ...
    qcexport.general_copy(
        "result", staging_storage, production_storage, full_pk_map, options, {"status": "error"}, {"id": "desc"}, 10
    )
    qcexport.general_copy(
